...
public class AMF0 {
    ...
    private static void encodeString(final ByteBuf out, final String value) {
        ...
        out.writeShort((short) bytes.length);  // 写入长度
        out.writeBytes(bytes);  // 后面写入字节
    }
    ...
    private static Object decode(final ByteBuf in, final Type type) {
        switch (type) {
            ...
            case ARRAY:
                // [type][4,length]{[type][object][type][object][]}
            ...
            case OBJECT:
                ...
                if (type == Type.MAP) {
                    ...
                    if (count > 0) {
                        log.info("non-zero size for MAP type: {}", count);
                    }
                } else {
                    count = 0;
                    map = new Amf0Object();
                ...
                while (in.isReadable()) {
                    ...
                    if (Arrays.equals(endMarker, OBJECT_END_MARKER)) { // 读取到结束符
                        in.skipBytes(3);
    ...
    private static String decodeString(final ByteBuf in) {
        final short size = in.readShort(); // 读取长度
        final byte[] bytes = new byte[size];  // 读取字节
        in.readBytes(bytes);
